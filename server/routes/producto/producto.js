...
app.post('/', async(req, res) => {
    ...
    try {
        ...
        if (productoEncontrado) return res.status(400).json({
            ok: false,
            resp: 400,
            msg: `El producto que se desea insertar con el nombre ${producto.strNombre} ya se encuentra registrada en la base de datos.`,
            cont: {
                Producto: productoEncontrado.strProducto
            }
        });
    ...
    try {
        ...
        if (err) {
            return res.status(400).json({
                ok: false,
                resp: 400,
                msg: 'Error: Error al actualizar producto.',
                cont: {
                    err
                }
            });
        ...
        const productoActualizado = await ProductoModel.findByIdAndUpdate(idProducto, { $set: nuevoProducto }, { new: true });
...
app.delete('/', async(req, res) => {
    ...
    try {
        ...
        res.status(500).send({
            estatus: '500',
            err: true,
            msg: 'Error: Error al eliminar a el producto.',
            cont: {
                err: Object.keys(err).length === 0 ? err.message : err
            }
        });
